...
const Assets = (props) => {
  ...
  const api = props.api;
  konsole.log('propsatAgingAssets',props);
  useEffect(() => {
    if(memberId !== ''){
  ...
  return (
    <div className="financialInformationTable">
      ...
      <p className="p3 generate-pdf-main-color"> Non-Qualified Financial Assets (Bank Accounts, CDs, Money Market Funds, Stocks, Bonds, Etc.):</p>
            ...
            userAgingAssests.map((asset, i) => {
              return (
                <tr key={i} className="text-center">
                  ...
                  <td className="tdd">{$AHelper.capitalizeAllLetters(asset.nameOfInstitution || "-")}</td>
      ...
      <br />
      <br />
      <p className="p3 generate-pdf-main-color">Qualified Assets (IRAs, 401(k)s, 403(b)s, etc.):</p>
      <table className="container-fluid">
            ...
            realUserAgingAssests.map((asset, i) => {
              return (
                <tr key={i}>
                  ...
                  <td className="tdd text-center">
                  {isNotValidNullUndefile(asset.isRealPropertys[0]?.purchasePrice) ? $AHelper.IncludeDollars(asset.isRealPropertys[0]?.purchasePrice) : "-"}
                  </td>
                  <td className="tdd text-center">
                  {isNotValidNullUndefile(asset.isRealPropertys[0]?.value)  ? $AHelper.IncludeDollars(asset.isRealPropertys[0]?.value) : "-"}
                  </td>
                  <td className="tdd text-center">
                    {(asset &&
                      asset.assetOwners != undefined &&  asset.assetOwners.length == 2) ? "Joint" :  asset.assetOwners.length == 1 ? 
                     $AHelper.capitalizeAllLetters( asset.assetOwners[0]?.ownerUserName) :
                      ""}
                  </td>
                </tr>
              );
      ...
      <table className="container-fluid">
        ...
        <tr>
          ...
          <td className="tdd text-center ps-3"> {userLifeInsurence && userLifeInsurence.length > 0 ? (<OtherInfo othersCategoryId={12} othersMapNatureId={userLifeInsurence[0]?.userLifeInsuranceId } FieldName={$AHelper.capitalizeAllLetters(userLifeInsurence[0]?.insuranceCompany)} userId={props?.memberId}/>):"-"} </td>
        
          {spouseId && <td className="tdd text-center ps-3">
        ...
        <tr>
          ...
          <td className="tdd text-center ps-2">{userLifeInsurence && userLifeInsurence.length > 0 ? ( <OtherInfo  othersCategoryId={23}  othersMapNatureId={userLifeInsurence[0]?.userLifeInsuranceId }  FieldName={userLifeInsurence[0]?.insuranceCompany}  userId={props?.memberId}/> ):"-"}</td>  
          {spouseId && <td className="tdd text-center ps-3">
        ...
        <tr>
          ...
          {spouseId && <td className="tdd text-center">
            {spouseLifeInsurence && spouseLifeInsurence.length > 0 ? (
              <input
                type="text"
                value={spouseLifeInsurence[0]?.policyStartDate == null ? "" : $AHelper.getFormattedDate(spouseLifeInsurence[0]?.policyStartDate)}
                className="border-0 text-center"
              ></input>
            ):"-"}
          </td>}
    ...
    {spouseLifeInsurence && spouseLifeInsurence.length > 0 ? spouseLifeInsurence[0].beneficiary.length > 0 &&   spouseLifeInsurence[0].beneficiary[0].beneficiaryUserId !== null && (
        ...
        <tbody>
          {businessInterestGetList &&
            businessInterestGetList.map((b, index) => {
              return (
                <tr>
                  <td className="tdd text-center">{$AHelper.capitalizeAllLetters(b.nameofBusiness || "-")}</td>
                  <td className="tdd text-center">{b.descOfBusiness || "-"}</td>
                  <td className="tdd text-center">{isNotValidNullUndefile(b.dateFunded) ? $AHelper.getFormattedDate(b.dateFunded):"-"}</td>
                  <td className="tdd text-center"> <OtherInfo  othersCategoryId={30}  othersMapNatureId={b?.userBusinessInterestId }  FieldName={b?.businessType}  userId={props?.memberId}/> </td>
                  <td className="tdd text-center">{(b.estimatedMarketValue) ? $AHelper.IncludeDollars(b.estimatedMarketValue): "-"}</td>
                  <td className="tdd text-center">{$AHelper.capitalizeAllLetters(b.ownershipType) || "-"}</td>
                  <td className="tdd text-center">{b.additionalDetails || "-"}</td>
                  <td className="tdd text-center">{b.taxIDNo || "-"}</td>
                </tr>
              );
            })}
        </tbody>
      ...
      <table className="container-fluid text-center">
        ...
        <tr>
          ...
          <td className="tdd text-center ps-3">
          ...
          {spouseId && <td className="tdd center ps-3">
            ...
            {longTermCareInsurancePolicies &&
              longTermCareInsurancePolicies.length > 0 ? (
                <input
                  type="type"
                  className="m-0 border-0 text-center "
                  value={isNotValidNullUndefile(longTermCareInsurancePolicies[0].policyStartDate) ? $AHelper.getFormattedDate(longTermCareInsurancePolicies[0].policyStartDate):" "}
                ></input>
              ):"-"}
          </td>
          {spouseId && <td className="tdd text-center">
            {spouseLongTermInsurencePolicies &&
              spouseLongTermInsurencePolicies.length > 0 ? (
                <input
                  type="type"
                  className="m-0 border-0 text-center"
                  value={isNotValidNullUndefile(spouseLongTermInsurencePolicies[0].policyStartDate) ? $AHelper.getFormattedDate(spouseLongTermInsurencePolicies[0].policyStartDate) : " "}
                ></input>
              ):"-"}
          </td>}
        </tr>
        ...
        <tr>
          ...
          <td className="tdd text-center">
            {longTermCareInsurancePolicies &&
              longTermCareInsurancePolicies.length > 0 ? (
                <p className="text-center ps-2">{longTermCareInsurancePolicies[0].planWithInflationRider == "true" ? "Yes" : "No"}</p>
              ):"-"}
          </td>
          {spouseId && <td className="tdd text-center">
            {spouseLongTermInsurencePolicies &&
              spouseLongTermInsurencePolicies.length > 0 ? (
                <p className="text-center ps-2">{spouseLongTermInsurencePolicies[0].planWithInflationRider == "true" ? "Yes" : "No"}</p>
              ):"-"}
          </td>}
        </tr>
        <tr>
