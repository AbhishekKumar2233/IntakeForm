...
import MonthlyExpenses from './currentExpenses/monthlyExpenses'
import NonMonthlyExpenses from "./currentExpenses/NonMonthlyExpences";
    ...
    <div className="financialInformationTable">
          ...
          {userLiability && userLiability.map((liability, index) => {
            return (
              ...
              >
                ...
                <td className="tdd">{isNotValidNullUndefile(liability.payOffDate) ?  $AHelper.getFormattedDate(liability.payOffDate):"-"}</td>
                <td className="tdd">{uscurencyFormate(liability.outstandingBalance) || "-"}</td>
                <td className="tdd">{uscurencyFormate(liability.paymentAmount) || "-"}</td>
              </tr>
            );
          })}
        </tbody>
      ...
      <div className="container-fluid">
        ...
        <div className="row">
          ...
          <MonthlyExpenses memberId={memberId} refrencePage={props?.refrencePage}></MonthlyExpenses>
          ...
          <ul className="pt-3 ps-3"><li className="generate-pdf-main-color" style={{fontSize:"13px",fontWeight:"600"}}>Non-Monthly Expenses</li></ul> 
            <NonMonthlyExpenses memberId={memberId}/>
          </div>
        </div>
      </div>
    </div>
