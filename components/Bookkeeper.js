...
import { Book } from "./control/Constant";
...
export class Bookkeeper extends Component {
  ...
  componentDidMount() {
    konsole.log("propospsBook",this.props)
    ...
    // userId: this.props.docUsr?.userId;
  }
  ...
  updateProCategories = (proCategories) =>{
    ...
    const toUpdateServiceProvider = (proCategories || [])?.map((proCategory) => ({
      ...
      proTypeId: proCategory?.proTypeId || 12,
      proSubTypeId: null,
                    ...
                    if (response.data.data.userSubjects.length !== 0) {
                      ...
                      for (  let i = 0;  i < formlabelData[label].response.length;  i++) {
                        ...
                        if (  formlabelData[label].response[i].responseId ===responseData.responseId) {
                          konsole.log("responseDataresponseData",responseData,responseData.responseNature)
  ...
  handleChange = (event) => {
    ...
    konsole.log("lkabel774",this.state.formlabelData.label774)
    ...
    if (eventName == "happy") {
      this.setState({
        ...
        happy: {
          ...
          subjectId: this.state.formlabelData.label774 &&  this.state.formlabelData.label774.questionId,
          userSubjectDataId: this.state.formlabelData.label774 ? this.state.formlabelData.label774.userSubjectDataId : 0,
        },
      });
      ...
      this.setState({
        ...
        accountant: {
          responseId: eventId,
          subResponseData: eventValue,
          subjectId:  this.state.formlabelData.label775 && this.state.formlabelData.label775.questionId,
          userSubjectDataId: this.state.formlabelData.label775 ? this.state.formlabelData.label775.userSubjectDataId : 0,
        },
      });
      ...
      this.setState({
        ...this.state,
        longUsed: {
          userSubjectDataId: this.state.formlabelData.label910
            ? this.state.formlabelData.label910.userSubjectDataId
            : 0,
          responseId: eventId,
          subResponseData: eventValue,
          subjectId:
            this.state.formlabelData.label910 &&
            this.state.formlabelData.label910.questionId,
        },
      });
    }
    else{
  ...
  handleMetataDataSubmit = () => {
    ...
    konsole.log("happyhappyhappyhappy",happy)
    ...
    this.props.dispatchloader(true);
    $CommonServiceFn.InvokeCommonApi("POST",
  ...
  handleUpdateMetaDataSubmit = () => {
    ...
    konsole.log("happyhappyhappya",happy)
    ...
    konsole.log("happyhappyhappya",accountant)
    ...
    konsole.log("happyhappyhappya",company)
    let longUsed = {
      userSubjectDataId: this.state.longUsed.userSubjectDataId,
      subjectId: this.state.longUsed.subjectId,
      subResponseData: this.state.longUsed.subResponseData,
    ...
    konsole.log("happyhappyhappya",longUsed)
    ...
    konsole.log("metadatametadata",happy,totinptary)

    konsole.log("metadatametadata",updatePostData,this.props.docUsr.userId, JSON.stringify(totinptary));
     this.state.disable=true
    this.props.dispatchloader(true);

    $CommonServiceFn.InvokeCommonApi( "PUT", $Service_Url.putSubjectResponse,
     // + "?userId=" + this.state.userId
      updatePostData,(response) => {
        this.setState({ disable: false })
        // AlertToaster.success("Bookkeeper update successfully");

        this.props.dispatchloader(false);
        // konsole.log("Success res" + JSON.stringify(response));
        if (response) {
          this.state.disable=false
          konsole.log("putSubjectResponse",response)
          // alert("Saved Successfully");
          // this.getsubjectForFormLabelId(this.props.docUsr.userId);
          this.props.setBookeeparShow();
  ...
  //   konsole.log("DocuserId", userid, "addressId", addressid);
  ...
  handleDeleteContact = async (userid, contactId) => {
    userid = userid || this.state.docUserId;
    let ques = await this.context.confirm(
      true,
    ...
    if (ques) {
      ...
      $CommonServiceFn.InvokeCommonApi(
        "DELETE",
        $Service_Url.deleteContactPath + userid + "/" + contactId,
        "",
        (response) => {
          this.setState({ disable: false })
          this.props.dispatchloader(false);
  ...
  handleAccountantSubmit = () => {
    ...
    if (this.props.toUpdate !== true || (this.props.checkSameProff != true && this.state.sameWithPrimary == true) || (this.props.docUsr?.businessName != this.state.businessName) || (this.props.docUsr?.businessTypeId != this.state.businessTypeId) || (this.props.docUsr?.businessTypeId == "999999") || (this.props.docUsr?.websiteLink != this.state.websiteLink)) {
      ...
      $CommonServiceFn.InvokeCommonApi(
        "POST",
        $Service_Url.postProfessionalUserMapping,inputdata,(response) => {
          this.setState({disable:false})
          konsole.log("Success res" + JSON.stringify(response));
          this.props.dispatchloader(false);
          if (response) {
            // AlertToaster.success("Bookkeeper saved successfully");
            // alert("Bookkeeper saved successfully");
  ...
  //   $postServiceFn.handleAddprofessinal(
  //     professUserId.proUserId,
  //     this.state.userId,
  //     professUserId.proSerDescId || professUserId.protypeId,
  //     this.state.loggedInUser,
  //     (res) => {
  ...
  handlePutName = () => {
    // alert("lklklkkllkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkkk");
    let physicianInfo = {
      userId: this.state.docUserId,
    ...
    $postServiceFn.putUserInfo(physicianInfo, (res) => {
      AlertToaster.success(`Bookkeeper ${this.props.toUpdate == true ? "updated" : "saved"} successfully`);
  ...
  render() {
    konsole.log("formlabelData",this.state.formlabelData)
    konsole.log("longUsedlongUsed",this.state.happy)
    konsole.log("adiver", this.props);
    konsole.log("eveeeeeeeeeeeeeee",this.state)
    ...
    return (
      <>
        ...
        {/* <Modal
          show={this.state.show == true ? false : this.props.show}
          size="lg"
          centered
          onHide={this.props.setBookeeparShow}
          animation="false"
          backdrop="static"
        >
          <Modal.Header closeButton closeVariant="white">
            <Modal.Title>BookKeeper</Modal.Title>
          </Modal.Header>
          <Modal.Body className="pb-5 pt-4"> */}
            ...
            {/* <Row>
              <Col lg="12">
                  <ContactListComponent userId={this.props.docUsr.userId} contactDetails={this.contactDetails} />
              </Col>
            </Row> */}
            ...
            <Row className="m-0 mb-3">
              ...
              <Row className="m-0 mb-3 border pb-2 pt-1 ms-10px">
                  <Col xs sm="10" lg="9" id="healthInsurance1">
                    <label className="mb-1">
                      {`Does your ${this.state.maritalStatusId == 2 ? "partner": "spouse"} use the same Bookkeeper?`}{" "}
                    </label>
              ...
              <Form.Group as={Row} className="mb-3">
                ...
                <p>
                      {this.state.formlabelData.label910 &&
                        this.state.formlabelData.label910.question}{" "}
                    </p>
                  {this.state.formlabelData.label910 &&
                    this.state.formlabelData.label910.response.map(
                      (response) => {
                        return (
                          <>
                            <Form.Control
              ...
              <div className="ms-10px w-100 border p-2">
                <Form.Group as={Row} className="mb-3">
                  <Col xs="12" sm="12"  lg="12">
                    <p>
                      {this.state.formlabelData.label774 &&
                        this.state.formlabelData.label774.question}{" "}
                    </p>
                    <div className="d-flex align-items-center justify-content-start">
                      {this.state.formlabelData.label774 &&
                        this.state.formlabelData.label774.response.map((response) => {
                ...
                <Form.Group as={Row} className="mb-3">
                  <Col xs="12" sm="12" lg="12">
                    <p>
                      {this.state.formlabelData.label775 &&
                        this.state.formlabelData.label775.question}
                    </p>
                    <div className="d-flex flex-wrap align-items-center justify-content-start">
                      {this.state.formlabelData.label775 &&
                        this.state.formlabelData.label775.response.map(
                          (response) => {
                            return (
                              <>
                                <Form.Check
          ...
          <div className="ms-10px mt-4">
            ...
            className="theme-btn" onClick={this.handleAccountantSubmit}
